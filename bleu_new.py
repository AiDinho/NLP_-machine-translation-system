...
def bleu_stats_4ref(sentence, references):
  yield len(sentence)
  yield min([len(r) for r in references], key=lambda l: abs(l - len(sentence)))
  for n in xrange(1,5):
    s_ngrams = Counter([tuple(sentence[i:i+n]) for i in xrange(len(sentence)+1-n)])
    r0_ngrams = Counter([tuple(references[0][i:i+n]) for i in xrange(len(references[0])+1-n)])
    r1_ngrams = Counter([tuple(references[1][i:i+n]) for i in xrange(len(references[1])+1-n)])
    r2_ngrams = Counter([tuple(references[2][i:i+n]) for i in xrange(len(references[2])+1-n)])
    r3_ngrams = Counter([tuple(references[3][i:i+n]) for i in xrange(len(references[3])+1-n)])
    r_ngrams = r0_ngrams | r1_ngrams | r2_ngrams | r3_ngrams
    yield max([sum((s_ngrams & r_ngrams).values()), 0])
    yield max([len(sentence)+1-n, 0])
