...
def train(nbest, fr_file, en_file):
    ...
    for line in open(en_file):
        reference_english.append(line.strip())
    for i, j in fr_sentence.iteritems():
        en_list = [(x.translation, x.features, bleu(tuple(bleu_stats(x.translation, reference_english[i])))) for x in nbest[str(i)]]
        nbest[str(i)] = en_list
