# Identify the source phrases in the given dev/test set and filter the phrase file for the source phrases
...
from Trie import SimpleSuffixTree
...
def loadTrie(ruleFile):
    ...
    for line in iF:
        ...
        if prev_src != src:
            prev_src = src
            if new_trie is None:
                new_trie = SimpleSuffixTree(src, MAX_PHR_LEN)
            else:
...
def filterRules(dataFile):
    '''Filter the partial rule (phrase) file for the specified dev/test set before computing p(f|e) and p(e|f)'''
    ...
    try:
        for line in rF:
            ...
            for i in range ( len(words) ):
                for j in range(i, min(i + MAX_PHR_LEN, len(words))):
...
def main():
    ...
    # load the rules (phrases) to a suffix trie
    loadTrie(ruleFile)

    # filter the consolidated rules (phrases) that match devfile in trie into tempFile
    filterRules(devFile)
